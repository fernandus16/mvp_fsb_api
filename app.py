...
def get_produtos():
    ...
    Retorna uma representação da listagem de produtos e também o total eesperado a ser gasto.
    """
    ...
    if not produtos:
        ...
        return {"produtos": [], "total_gasto": .0}, 200
    else:
...
def get_produto(query: ProdutoBuscaSchemaPorId):
    ...
    Retorna uma representação dos produtos e comentários associados.
    """
    produto_id = query.id_produto
    produto_nome = query.nome
    logger.debug(f"Coletando dados sobre produto de nome:{produto_nome}")
...
@app.get('/produto_por_nome', tags=[produto_tag],
         responses={"200": ProdutoViewSchema, "404": ErrorSchema})
def get_produto_por_nome(query: ProdutoBuscaSchemaPorNome):
    """Faz a busca por um Produto a partir do nome do produto

    Retorna uma representação dos produtos e comentários associados.
    """
    produto_nome = query.nome.upper()
    logger.debug(f"Coletando dados sobre produto #{produto_nome}")
    # criando conexão com a base
    session = Session()
    # fazendo a busca
    produto = session.query(Produto).filter(func.upper(Produto.nome) == produto_nome).first()
